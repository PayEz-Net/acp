  ...
  return (
    <div className="border-t border-slate-800 pt-3">
      ...
      {loading ? (
        ...
        >
          {lines.map((line, i) => (
            <div key={`${line}-${i}`} className="text-[10px] text-slate-400 whitespace-pre leading-4">
              {line}
            </div>
          ))}
        </div>
...
function PoolProfileItem({
  ...
  const handleHire = async () => {
    ...
    try {
      ...
      if (useAuthStore.getState().authFlowState !== AuthFlowState.AUTHENTICATED) {
        setError('Not signed in — please log in again to send mail.');
        setHiring(false);
        return;
      }
