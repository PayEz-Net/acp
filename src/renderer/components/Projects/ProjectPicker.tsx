...
//
// switchProject is the REAL atomic switch (verified E2E): store.switchProject
// → electronAPI.switchProject(id, name) → main/project-switch.ts POSTs
// /v1/projects/current, writes the next-boot overlay, then app.relaunch() +
// app.exit(0) so the cold boot re-instantiates the picked project's team.
// On cloud/network failure no relaunch happens and switchError surfaces the
// verbatim message inline. (The former "fail-loud stub" was replaced in Wave
// C/2 Commit D — this comment was stale.)
export function ProjectPicker({ isOpen, onClose }: ProjectPickerProps) {
  ...
  // Name lookup for the workdir surface header (selection may not be the
  // active pointer yet on the first-pick path).
  const targetProjectNameForId = (id: number): string =>
    projects.find((p) => p.id === id)?.name ?? activeProject?.name ?? 'this project';
  ...
  const handleStart = async () => {
    ...
    // Non-null capture so the nested triggerStart closure narrows to `number`.
    ...
    // SPEC-workdir-invalid §3.5 — the single open-time instantiation gate.
    // Validate the project's repo_path ONCE, before any spawn. Invalid →
    // refuse to start (spawn ZERO agents), raise the inline correction surface,
    // and report false so the caller does NOT mark the picker started / close.
    // Returns true only when the path resolves and the spawn fan-out has fired.
    const triggerStart = async (): Promise<boolean> => {
      // Read the freshly-hydrated active project (setCurrentProject ran on the
      // first-pick / cold-different paths just above; same-project path already
      // has it). Fall back to the picker list copy if the active pointer hasn't
      // caught up to the selection yet.
      const store = useProjectStore.getState();
      const proj =
        store.activeProject?.id === startProjectId
          ? store.activeProject
          : store.projects.find((p) => p.id === startProjectId) ?? store.activeProject;
      const repoPath = proj?.repo_path ?? '';

      const check = await window.electronAPI.validateWorkDir(repoPath);
      if (!check.ok) {
        // Invalid working folder — do NOT reseed (no spawn fan-out). Raise the
        // SHARED workdir-invalid slice (§3.4/§3.5 dedup seam): the App-level
        // WorkdirCorrection surface renders from it, the SAME surface the
        // orchestrator-path IPC listener feeds. The spawn happens only after
        // the path is corrected + persisted.
        useProjectStore.getState().setWorkdirInvalid({
          projectId: startProjectId,
          projectName: proj?.name ?? targetProjectNameForId(startProjectId),
          badPath: repoPath,
        });
        return false;
      }

      // SPEC-team-runtime §3.2 — couple reconcile-on-switch with the workdir
      // pre-flight gate at the SAME instantiation moment, BEFORE reseed. On a
      // fresh open (nothing running) this is a no-op (no prompt). If prior
      // in-session agents are running on the wrong provider, it surfaces the
      // confirm plan; reseed still proceeds to spawn any missing agents under
    ...
    // Stored mode + different project.
    // If we haven't started yet (cold boot), just set the project and
    // reseed lifecycle — no hard restart needed because nothing is running.
    // If already running, use the atomic switch flow with app relaunch
    // so the old project's team is fully torn down and the new one spawns
    // from a clean state.
    if (!pickerHasStarted) {
      setStarting(true);
      const success = await setCurrentProject(selectedProjectId);
    ...
    // Already running — atomic switch with hard restart.
    const targetProject = projects.find((p) => p.id === selectedProjectId);
    const confirmed = window.confirm(
      `ACP will restart to switch to '${targetProject?.name || 'the selected project'}'.\n\n` +
      `Your startup project will not be changed — you can update that anytime on idealvibe.online.`
    );
    if (!confirmed) {
      return;
    }
      ...
      <div className="w-[28rem] max-h-[80vh] bg-slate-900 border border-slate-700 rounded-lg shadow-2xl flex flex-col overflow-hidden">
        ...
        {pickerMode === 'stored-confirm' && !expanded && (
          <p className="px-6 pb-3 -mt-1 text-[11px] leading-relaxed text-slate-500 text-center">
            ACP launches this startup project on every start.
            Use “Pick another” to switch to one of your other projects.
          </p>
        )}
          ...
          <div className="border-t border-slate-700 px-3 py-2.5 flex items-center gap-2">
            ...
            {pickerMode === 'stored-confirm' && !expanded ? (
              <button
                ...
                title="Show your other projects to switch to"
              >
