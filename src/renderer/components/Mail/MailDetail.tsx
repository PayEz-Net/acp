...
import { X, Clock, User, AlertTriangle, Flag, Info } from 'lucide-react';
...
export function MailDetail({ message, actions, suggested, onClose, onAction }: MailDetailProps) {
  ...
  // Mail Muffler L2 (BAPert 7133): `info` = terminal/no-reply tier — surface the marker
  // in the reading pane so the reader knows it's safe to release without replying.
  const isInfo = message.importance === 'info';

  // Observe-only (BAPert 1369): human reads agent mail, cannot send or action.
  // All server-returned action buttons (reply, compose, acknowledge, forward,
  // archive, inbox) are removed G�� none are functional in the Electron sidebar.
  const observeActions: typeof actions = [];
    ...
    <div className="flex flex-col h-full bg-slate-900">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {isInfo && (
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium bg-slate-600/30 text-slate-400 rounded shrink-0">
      ...
      {/* Actions bar G�� observe-only: SEND actions (reply/compose) filtered out (BAPert 1369) */}
      {observeActions.length > 0 && (
        <div className="border-t border-slate-700 px-4 py-3 space-y-3">
          ...
          {/* Keyboard hints */}
          {observeActions.some(a => a.key) && (
            <div className="flex flex-wrap gap-2 text-[10px] text-slate-500">
              {observeActions.filter(a => a.key).map((action) => (
                <span key={action.key}>
