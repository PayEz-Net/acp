...
export enum AuthFlowState {
  ...
  // Session died MID-USE (was authenticated, main broadcast session-dead at the
  // 3-day mark / max-refresh-failures). Distinct from UNAUTHENTICATED so the app
  // stays MOUNTED (agent panes + their context survive) and the avatar shows the
  // 🔴 "re-login" affordance — instead of unmounting to LoginScreen (which killed
  // every pane). In-place re-login (loginWithOAuth) recovers from here. (WO: in-app
  // re-login + auth indicator.)
...
interface AuthStore {
  ...
  /** Mark the session dead MID-USE — keeps `user` so the app stays mounted (panes
   *  survive) and the avatar shows 🔴 re-login. Use instead of logout() for the
   *  main session-dead broadcast / detected mid-session expiry. */
...
export const useAuthStore = create<AuthStore>((set, _get) => ({
  ...
  /**
   * Mark the session dead MID-USE (main broadcast session-dead / detected expiry).
   * Unlike logout(), this KEEPS `user` and sets SESSION_EXPIRED — so App.tsx keeps
   * the app mounted (agent panes + context survive) and the avatar shows 🔴 with an
   * in-place re-login, instead of unmounting to LoginScreen. No-op if already expired
   * or not authenticated (don't clobber a cold-start / explicit-logout state).
   */
  markSessionExpired: () => {
    const { authFlowState, user } = _get();
    if (authFlowState === AuthFlowState.SESSION_EXPIRED) return;
    if (!user) return; // never authed (cold start) — let the normal UNAUTHENTICATED→LoginScreen path own it
    console.warn('[AuthStore] Session expired mid-use — staying mounted, avatar → re-login (panes preserved)');
    set({
      authFlowState: AuthFlowState.SESSION_EXPIRED,
      isLoading: false,
      error: null,
    });
  },
