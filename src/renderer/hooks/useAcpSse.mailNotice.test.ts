...
describe('routeMailNotice wiring (WO 11517)', () => {
  it('pty-echo: a PTY-bridge agent gets the self-instructing notice as an info line', () => {
    useAppStore.setState({
      agents: [{ id: '1', name: 'NextPert', terminalId: 't-pty' } as never],
    });

    routeMailNotice('NextPert', 'BAPert', 'WORK ORDER', 9101);
  ...
  it('acp-inject: a live ACP session injects first and echoes only after acceptance', async () => {
    ...
    routeMailNotice('NextPert', 'BAPert', 'WORK ORDER', 9102);
    ...
    expect(injectedText).toContain('Read it NOW with: curl');
