...
/**
 * Map a TYPED spawnAgent error to a 422 the renderer can surface
 * (SPEC-workdir-invalid §3.4 + SPEC-team-runtime §3.3). Returns true if it
 * sent a typed response, false to let the caller rethrow to the generic 500
 * (never widen the typed set to masquerade an unexpected error). Both blocked
 * cases are well-formed-request-but-cannot-instantiate, so 422 (not 500):
 *   • WORKDIR_INVALID  — the project's working folder is missing on this box.
 *   • RUNTIME_NOT_SET  — the active project has no team runtime; we BLOCK
 *                        rather than launch a silent default provider.
 */
function trySendTypedSpawnError(res: http.ServerResponse, err: unknown, agentName: string): boolean {
  if (err instanceof WorkDirError) {
    console.error(`[Lifecycle] WorkDirError spawning ${agentName}: ${err.message}`);
    sendJson(res, 422, { code: 'WORKDIR_INVALID', message: err.message, work_dir: err.workDir, agent_name: agentName });
    return true;
  }
  if (err instanceof RuntimeNotSetError) {
    console.error(`[Lifecycle] RuntimeNotSetError spawning ${agentName}: ${err.message}`);
    sendJson(res, 422, { code: 'RUNTIME_NOT_SET', message: err.message, project_id: err.projectId, agent_name: agentName });
    return true;
  }
  return false;
}
  ...
  server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url === '/internal/pty/spawn' && req.method === 'POST') {
        ...
        // Per-spawn runtime from acp-api (project-driven). Validated against
        // the allowed set; unknown/absent → undefined. SPEC-team-runtime §3.3:
        // for a project-scoped spawn (projectId set), an undefined runtime makes
        // spawnAgent THROW RuntimeNotSetError → typed 422 below (NOT a global
        // fallback). The global only applies to a genuinely no-project spawn.
        ...
        if (existing) {
          ...
          // is exactly what left QAPert-NightHawk/SEOPert on kimi in a claude
          // team. If a team runtime was requested and the existing terminal's
          // provider differs, restart-to-conform: kill the stale PTY and
          // respawn under the team runtime, returning the new terminalId.
          // When no runtime was requested (no-project / legacy IPC) or the
          // provider already matches, reattach as before — NO needless churn
          // (§5: conforming agents are left untouched).
          if (runtime && existing.provider !== runtime) {
            console.log(`[Lifecycle] Reattach-conform ${agentName}: live provider '${existing.provider}' != team runtime '${runtime}' — restarting to conform`);
            ...
            console.log(`[Lifecycle] Conformed ${agentName}: ${existing.provider} -> ${runtime} (${conformedId})`);
            sendJson(res, 200, {
              terminalId: conformedId,
              status: 'conformed',
              conformed_from: existing.provider,
              conformed_to: runtime,
            });
            return;
          }
        ...
        try {
          ...
          // the renderer surfaces an actionable banner. Anything else rethrows
          // to the generic 500 — never widen the typed set to masquerade an
          // unexpected error.
          if (trySendTypedSpawnError(res, spawnErr, agentName)) return;
          throw spawnErr;
        }
