...
import { VIBE_API_URL, IDP_URL } from './cloud-endpoints';
...
// VIBE client identifier (non-secret, like an OAuth client_id). In the OSS
// repo this is NOT baked — you supply YOUR OWN via env. An official PayEz
// build may inject its own client id at build/runtime via VIBE_CLIENT_ID.
// Absent => acp-api's required('VIBE_CLIENT_ID') hard-throws, which is the
// DESIRED behavior (no fallback/soften): configure your client id. The
// VIBE_HMAC_KEY signing secret is NEVER baked or injected.
const BAKED_VIBE_CLIENT_ID = process.env.VIBE_CLIENT_ID || '';
  ...
  if (isPackaged) {
    ...
    apiProcess = spawn(process.execPath, [serverScript], {
      ...
      env: {
        ...
        VIBE_CLIENT_ID: BAKED_VIBE_CLIENT_ID,
