...
export const useMailStore = create<MailStore>((set, get) => ({
  ...
  fetchAllInboxes: async (agents, projectId) => {
    const { fetchInbox } = get();
    await Promise.all(agents.map((agent) => fetchInbox(agent, projectId)));
  },
  ...
  executeAction: async (action) => {
    ...
    switch (action.action) {
      ...
      case 'check_inbox':
        // Refresh all mailboxes — extract bare agent names from composite keys
        const agents = Object.keys(get().mailboxes).map(extractAgentFromKey);
        const uniqueAgents = [...new Set(agents)];
        await Promise.all(uniqueAgents.map(agent => fetchInbox(agent)));
        break;
