...
// Hub URL is NOT resolved here. The renderer resolves nothing — it asks
// the single main-process authority (cloud-endpoints.ts) via IPC at
// connect() time. The old `import.meta.env.VITE_ACP_HUB_URL || dev-93`
// line baked dev-93 into every packaged renderer (VITE_ACP_HUB_URL was
// never set), silently pointing prod mail-push at an unreachable dev box
// (masked only because mail also REST-polls the local sidecar).
...
export class AgentMailSignalRClient {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      // Resolve nothing locally — the main-process authority decided.
      const { hubUrl } = await window.electronAPI.getCloudEndpoints();
