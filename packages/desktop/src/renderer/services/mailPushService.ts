...
export class MailPushService {
  ...
  private handleNotification(notification: MailPushNotification): void {
    ...
    // 1. Refresh the inbox for the recipient agent (scoped to active project)
    if (data.to_agent) {
      mailStore.fetchInbox(data.to_agent, projectId);
    }
    ...
    if (data.to_agent) {
      ...
      // TODO(QAPert-L3): backend push payload should include project_id so
      // injectMessage can use the exact project scope. For now we fall back
      // to the active project, which is correct in single-project mode.
      const projectId = useProjectStore.getState().activeProject?.id;
      ...
      injectMessage(data.to_agent, message, projectId);
    }
  }
