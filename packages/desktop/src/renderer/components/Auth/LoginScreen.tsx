...
// Provider display metadata. Names match the slugs IDP returns.
const PROVIDER_META: Record<string, { label: string; iconUrl?: string }> = {
...
export function LoginScreen() {
  const { login, loginWithOAuth, error, isLoading } = useAuthStore();
  ...
  // Load enabled OAuth providers from IDP client config. Failures are
  // intentionally silent â€” if the config can't be fetched we just don't
  // show OAuth buttons, password login still works.
  useEffect(() => {
    let cancelled = false;
    getEnabledOAuthProviders()
      ...
      .catch((err) => {
        console.warn('[LoginScreen] OAuth providers unavailable:', err);
      });
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center text-slate-500 text-xs mt-3 flex items-center justify-center gap-2">
          <button
            type="button"
