...
const CONFIG = {
  dev: {
    apiUrl: 'http://127.0.0.1:32786',
    clientId: process.env.VIBE_CLIENT_ID || '',
    hmacKey: process.env.VIBE_HMAC_KEY || ''
  },
  prod: {
    apiUrl: 'https://api.idealvibe.online',
    clientId: process.env.VIBE_CLIENT_ID_PROD || process.env.VIBE_CLIENT_ID || '',
    hmacKey: process.env.VIBE_HMAC_KEY_PROD || process.env.VIBE_HMAC_KEY || ''
  }
};

const cfg = CONFIG[IS_PROD ? 'prod' : 'dev'];

if (!cfg.clientId || !cfg.hmacKey) {
  console.error(`ERROR: VIBE_CLIENT_ID${IS_PROD ? '_PROD' : ''} and VIBE_HMAC_KEY${IS_PROD ? '_PROD' : ''} must be set in the environment.`);
  process.exit(1);
}
