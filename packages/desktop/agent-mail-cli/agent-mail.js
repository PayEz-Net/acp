...
const ENVIRONMENTS = {
  prod: {
    apiUrl: process.env.VIBE_API_URL ? `${process.env.VIBE_API_URL}/v1/agentmail` : 'https://api.idealvibe.online/v1/agentmail',
    clientId: process.env.VIBE_CLIENT_ID || '',
    secretKey: process.env.VIBE_HMAC_KEY || ''
  },
  test: {
    apiUrl: process.env.VIBESQL_URL ? `${process.env.VIBESQL_URL}/v1/agentmail` : 'http://localhost:5173/v1/agentmail',
    clientId: process.env.VIBE_CLIENT_ID || '',
    secretKey: process.env.VIBE_HMAC_KEY || ''
  }
};
...
Environments:
  --prod    api.idealvibe.online (or VIBE_API_URL)
  --test    localhost (or VIBESQL_URL)
`);
}
