...
// Environment configs — credentials must come from env vars
//   ACP_VIBE_CLIENT_ID + ACP_VIBE_CLIENT_SECRET (prod)
//   ACP_VIBE_CLIENT_ID_TEST + ACP_VIBE_CLIENT_SECRET_TEST (test)
const ENVIRONMENTS = {
  prod: {
    apiUrl: 'https://api.idealvibe.online/v1/agentmail',
    clientId: process.env.ACP_VIBE_CLIENT_ID || '',
    secretKey: process.env.ACP_VIBE_CLIENT_SECRET || ''
  },
  test: {
    apiUrl: 'http://localhost/v1/agentmail',
    clientId: process.env.ACP_VIBE_CLIENT_ID_TEST || 'PLACEHOLDER_TEST_CLIENT',
    secretKey: process.env.ACP_VIBE_CLIENT_SECRET_TEST || 'PLACEHOLDER_TEST_SECRET'
  }
};
...
async function main() {
  ...
  if (!config.clientId || !config.secretKey || config.clientId.startsWith('PLACEHOLDER_')) {
    const suffix = parsed.env === 'test' ? '_TEST' : '';
    console.error(`ERROR: ACP_VIBE_CLIENT_ID${suffix} and ACP_VIBE_CLIENT_SECRET${suffix} must be set in the environment.`);
    process.exit(1);
  }
