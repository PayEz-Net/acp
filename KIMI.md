...
- **Vite** — Build tool
- **xterm.js** + **node-pty** — Terminal emulation (each pane runs an AI agent session)
...
Each pane spawns an AI agent session via node-pty and auto-injects `"report as {AgentName}"`.

### AI Provider Selection

The app supports both **Kimi** and **Claude** as AI providers:

| Provider | Command | Features |
|----------|---------|----------|
| **Kimi** (default) | `kimi` | Uses `.agents/skills/` for project context |
| **Claude** | `claude --dangerously-skip-permissions` | Uses `.claude/skills/`, has effort levels |

**To switch providers:**
1. Open Settings (gear icon)
2. Go to "AI Provider" section
3. Select Kimi or Claude
4. Kill and restart agents (the change applies to new sessions)

**Startup differences:**
- **Kimi**: Simple `kimi` command, detects `> ` prompt
- **Claude**: `claude --dangerously-skip-permissions --effort {level}`, detects `❯` prompt

The `claudeEffort` setting (low/medium/high/max) only applies when provider is 'claude'.
...
- **ACP Backend API**: `E:\Repos\acp` (port 3001)
- **Agent Mail**: `https://api.idealvibe.online` (enterprise auth)
- **VibeSQL**: `http://10.0.0.93:5173`
