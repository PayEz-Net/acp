...
You are about to assume the identity of **Aurum**.
...
```bash
curl -s "http://127.0.0.1:3001/v1/agents/Aurum/profile" -H "X-ACP-Agent: Aurum"
```
...
```bash
curl -s "http://127.0.0.1:3001/v1/mail/inbox/Aurum?unread=true" -H "X-ACP-Agent: Aurum"
```
